...
#define AFILE "debuginputs/a.bin"
#define BFILE "debuginputs/b.bin"
#define GFILE "debuginputs/golden.bin"
#else
#define NUM_ELEMENTS (512*512)
#define LEADING_DIM 512
#define AFILE "inputs/a.bin"
#define BFILE "inputs/b.bin"
#define GFILE "inputs/golden.bin"
#endif
...
main(int argc, char*argv[]) {
   ...
   read_success = read_from_file(AFILE, a, NUM_ELEMENTS);
   if(read_success == 1) {
	read_success = read_from_file(BFILE, b, NUM_ELEMENTS);
	if (read_success == 1) {
	   matmul(a, b, c, LEADING_DIM,argc,argv);
	}
   }
   read_success = read_from_file(GFILE, a, NUM_ELEMENTS);
   if(read_success == 1) {
        double diff = 0; 
...
//   write_success = write_to_file(CFILE, c, NUM_ELEMENTS);
#ifdef DEBUG
